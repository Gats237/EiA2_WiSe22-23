...
(function (A081_Vogelhaus) {
    ...
    function snowPos() {
        for (let i = 0; i < 250; i++) {
            let snowposX = ctx.canvas.width * Math.random();
            let snowposY = ctx.canvas.height * Math.random() * (-2);
    ...
    function posTrees() {
        for (let i = 0; i < 42; i++) {
            let stammhöhe = 0.55 * ctx.canvas.height;
            let positionbaum = Math.random() * ctx.canvas.width;
    ...
    function drawTrees(_postion) {
        ...
        for (let i = -2; i < 3; i++) {
            ...
            let gradient1 = ctx.createLinearGradient(0, 1, 0, 100);
...
})(A081_Vogelhaus || (A081_Vogelhaus = {}));
//# sourceMappingURL=A081_Vogelhaus.js.map